...
RECO_SPACE = odl.uniform_discr(min_pt=[-DOMAIN_L, -DOMAIN_L], max_pt=[DOMAIN_L, DOMAIN_L],
                               shape=[N_IMG, N_IMG], dtype='float32')
...
def load_calibration_disk(file_name):
    ...
    calibration_disk = np.array(mat) / 255.0
    return calibration_disk


def geom_with_par(par):
    ...
    geometry_device = odl.tomo.geometry.conebeam.FanBeamGeometry(angle_dev_partition, DETECTOR_DEV_PARTITION,
                                                                 src_radius=src_r,
                                                                 det_shift_func=lambda angle: np.array(
                                                                     [[0, det_shift]]),
                                                                 src_shift_func=lambda angle: np.array(
                                                                     [[0, src_shift]]),
                                                                 det_radius=det_r,
...
def estimate_parameters(sino, gr_truth_img, gr_truth_img_flipped):
    ...
    res = optimize.differential_evolution(obfun, bnds, (sino, gr_truth_img, gr_truth_img_flipped))
    parameters = res.x
